...
<div class="page-header">
  ...
  <a href="<?= url("/public/destinations.php") ?>" class="btn secondary" style="font-size:13px;">← Quản lý điểm đến</a>
</div>
...
async function openContact(id) {
  ...
  // Mark read
  fetch(`${BASE}/admin/contact_mark_read.php`, {
  ...
  const [repliesData, msgData] = await Promise.all([
    ...
    fetch(`${BASE}/admin/contact_get.php?id=${id}`).then(r=>r.json()).catch(()=>({success:false}))
  ]);
...
async function sendReply(id) {
  ...
  try {
    const res  = await fetch(`${BASE}/admin/contact_reply.php`, {
      method:"POST", headers:{"Content-Type":"application/json"},
...
async function pollNewContacts() {
  try {
    const res  = await fetch(`${BASE}/admin/contact_poll.php?last_id=${lastMax}&_=${Date.now()}`);
    const data = await res.json();
