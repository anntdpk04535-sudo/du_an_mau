...
-- (trước đây được tạo bởi migrate_reviews.php)
--
...
--
-- Table structure for table `review_deletion_logs`
-- (trước đây được tạo tự động bởi api/review_delete.php)
--

CREATE TABLE `review_deletion_logs` (
  `id` int(11) NOT NULL,
...
ALTER TABLE `review_deletion_logs`
  ADD PRIMARY KEY (`id`),
...
-- (trước đây được tạo bởi migrate_contacts.php)
--
...
--
-- Bổ sung cột `address`, `transport` cho `itinerary_items`
-- (trước đây được tạo bởi migrate.php)
--

ALTER TABLE `itinerary_items`
  ADD COLUMN `address` VARCHAR(255) DEFAULT NULL AFTER `activity`,
  ADD COLUMN `transport` VARCHAR(255) DEFAULT NULL AFTER `address`;
