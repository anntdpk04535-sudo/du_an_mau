...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fullName === '' || $email === '' || $password === '') {
        $error = __('register_fill_all');
    } elseif ($password !== $passwordConfirm) {
        $error = __('register_password_mismatch');
    } elseif (strlen($password) < 6) {
        $error = __('register_password_min');
    } else {
        $db = getDB();
        $stmt = $db->prepare("SELECT id FROM users WHERE email = ?");
        $stmt->execute([$email]);
        if ($stmt->fetch()) {
            $error = __('register_email_exists');
        } else {
  ...
  <div class="auth-card">
    <div class="auth-image">
      <div class="auth-image-content">
        <h2><?= __('register_banner_title') ?></h2>
        <p><?= __('register_banner_desc') ?></p>
      </div>
    </div>
    <div class="auth-form-container">
      ...
      <form method="post">
        <div class="form-group">
          ...
          <input type="text" name="full_name" value="<?= e($_POST['full_name'] ?? '') ?>" required placeholder="<?= __('register_fullname_placeholder') ?>">
        </div>
        <div class="form-group">
          <label><?= __('email_label') ?></label>
          <input type="email" name="email" value="<?= e($_POST['email'] ?? '') ?>" required placeholder="<?= __('register_email_placeholder') ?>">
        </div>
        <div class="form-group">
          <label><?= __('password_label') ?></label>
          <input type="password" name="password" required placeholder="<?= __('register_password_placeholder') ?>">
        </div>
        <div class="form-group">
          <label><?= __('repassword_label') ?></label>
          <input type="password" name="password_confirm" required placeholder="<?= __('register_confirm_placeholder') ?>">
        </div>
        <button type="submit" class="btn"><?= __('register') ?></button>
      </form>

      <div class="auth-divider"><?= __('register_or_with') ?></div>
